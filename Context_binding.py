...
def clbk_laser(msg):
    ...
    regions_ = {
        ...
        'frontL': find_nearest(msg.ranges[35:55]),
        'frontR': find_nearest(msg.ranges[305:325]),
        'right': find_nearest(msg.ranges[210:225]),
...
#Creating a function to return the values of speed and direction from the POV of Right edge following
def right_fuzz():
    print("----------Right_____Edge------------")
    rf = regions_['frontR']
    r = regions_['right']
    rfs = sensor(rf).fuzzification()
    ...
    sr, dr = Defuzz_right(firing)
    return sr, dr

#Creating a function to return the values of Speed and direction from the POV of Obstical Avoidance
def obs_fuzz():
    print("----------OBS_____AVD------------")
    f = min(regions_['front1'], regions_['front2'])
    fr = regions_['frontR']
    fl = regions_['frontL']
    fs = sensor(f).fuzzification()
    fsr = sensor(fr).fuzzification()
    fsl = sensor(fl).fuzzification()
...
def movement():
    ...
    #The minimum distance seen by the right side
    RE = min(regions_['fright'], regions_['right'])
    #The minimum distance seen by the front side
    OA = min(regions_['frontL'], regions_['frontR'], regions_['front1'], regions_['front2'])

    #Calculating the membership values for both
    if OA<0.2:
        mvOA=1
    elif 0.2< OA < 1  :
        mvOA=(1-OA)/(1-0.2)
    ...
    #Getting the Speed and direction of the RE Follwoing
    sr, dr = right_fuzz()

    #Same here for the Obstical avoid
    sf, df = obs_fuzz()

    #protecting the system from errors
    if (mvRE + mvOA == 0):
        lin = 0.10
        ang = 0.0
    else:
        #calculating the speed of both methods by fuzzy logic using their membership values
        lin = (sr * mvRE + sf * mvOA) / (mvRE + mvOA)
        ang = (dr * mvRE + df * mvOA) / (mvRE + mvOA)

    print("Linear speed: ", lin, "Angular speed: ", ang)
    msg.linear.x = lin
    msg.angular.z = ang * -1
    return msg
